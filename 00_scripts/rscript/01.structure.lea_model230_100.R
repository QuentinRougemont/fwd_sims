...
input <- argv[1]
dir.create('04_lea/')
dir.create('04_lea/model230.slim.100')

output = vcf2geno(input,"04_lea/model230.slim.100/slim.100.genotype.geno")
#dir.create('04_lea/__mode_.slim._NB__')
#1. Estimating K
obj.at = snmf("./04_lea/model230.slim.100/slim.100.genotype.geno", K = 3, ploidy = 2, entropy = T,repetitions=10,alpha=10,percentage=0.05,
              CPU = 12, project = "new")
...
write.table(b, paste("lea.model230slim.100.mean.txt",sep="."), 
	quote=F, 
	row.names=F, 
	col.names=F)
