...
input <- argv[1]
dir.create('04_lea/')
dir.create('04_lea/model1.slim.1')

output = vcf2geno(input,"04_lea/model1.slim.1/slim.1.genotype.geno")
#dir.create('04_lea/__mode_.slim._NB__')
#1. Estimating K
obj.at = snmf("./04_lea/model1.slim.1/slim.1.genotype.geno", K = 3, ploidy = 2, entropy = T,repetitions=10,alpha=10,percentage=0.05,
              CPU = 12, project = "new")
...
write.table(b, paste("lea.model1slim.1.mean.txt",sep="."), 
	quote=F, 
	row.names=F, 
	col.names=F)
